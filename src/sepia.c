...
extern void sepia_four_pixels(float[static 4], float[static 4], float[static 4], void *);

static const float to_float[] = {
...
static unsigned char saturate(uint64_t x) {
  return x < 256 ? x : 255;
}
...
static struct pixel sepia_one_pixel(struct pixel pxl) {
  ...
  return (struct pixel) {
      saturate((uint64_t)(
          to_float[pxl.r] * c[0][0] +
          to_float[pxl.g] * c[0][1] +
          to_float[pxl.b] * c[0][2]
      )),
      saturate((uint64_t)(
          to_float[pxl.r] * c[1][0] +
          to_float[pxl.g] * c[1][1] +
          to_float[pxl.b] * c[1][2]
      )),
      saturate((uint64_t)(
          to_float[pxl.r] * c[2][0] +
          to_float[pxl.g] * c[2][1] +
          to_float[pxl.b] * c[2][2]
      ))
  };
}

struct image sepia_c_inplace(struct image img) {
  struct image new_img = image_create(img.dims);
  for (uint32_t i = 0; i < img.dims.width * img.dims.height; i++) {
    new_img.data[i] = sepia_one_pixel(img.data[i]);
  }
  return new_img;
}
...
struct image sepia_sse(struct image img) {
  struct image new_img = image_create(img.dims);
  float r[4], g[4], b[4];
  uint64_t i, size = img.dims.width * img.dims.height;
  for (i = 0; i < size; i += 4) {
    for (uint64_t j = 0; j < 4; j++) {
      r[j] = to_float[img.data[i + j].r];
      g[j] = to_float[img.data[i + j].g];
      b[j] = to_float[img.data[i + j].b];
    }
    sepia_four_pixels(r, g, b, &new_img.data[i]);
  }
  if (size % 4 != 0)
    for (i = i - 4; i < size; i += 1) {
      new_img.data[i] = sepia_one_pixel(img.data[i]);
    }
  return new_img;
}
